...
export default function GeneralDetail({ params }: { params: { id: string } }) {
  ...
  const generalMissions = apiMissions.length > 0
    ? apiMissions.filter((m) => m.agent_id === params.id || m.agent_id === params.id?.toUpperCase())
    : missions.filter((m) => m.assignedTo === general.id || m.assignedTo === general.id?.toLowerCase());

  const generalEvents = apiEvents.filter((e) => e.agent_id === params.id || e.agent_id === params.id?.toUpperCase());

  const generalRelationships = apiRelationships.filter(
    (r) => r.agent_a === params.id || r.agent_b === params.id ||
           r.agent_a === params.id?.toUpperCase() || r.agent_b === params.id?.toUpperCase()
  );
  ...
  return (
    <div className="max-w-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          {generalRelationships.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {generalRelationships
                ...
                .map((rel) => {
                  const otherId = rel.agent_a === params.id || rel.agent_a === params.id?.toUpperCase() ? rel.agent_b : rel.agent_a;
                  ...
                  <div key={m.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-4 p-2 md:p-3"
                    ...
                    <div className="flex gap-2 items-center">
                      <span className="font-pixel text-[7px] px-2 py-0.5" style={{
                        color: isComplete ? "#22c55e" : isActive ? "#fbbf24" : "#94a3b8",
                        backgroundColor: isComplete ? "#22c55e11" : isActive ? "#fbbf2411" : "#94a3b811",
                        border: `1px solid ${isComplete ? "#22c55e33" : isActive ? "#fbbf2433" : "#94a3b833"}`,
                      }}>
                        {statusUpper}
                      </span>
        ...
        <div className="rpg-panel p-3 md:p-4 lg:col-span-2">
          <h2 className="font-pixel text-[10px] text-throne-gold mb-4 text-rpg-shadow">⚡ EVENT LOG</h2>
        ...
        <div className="rpg-panel p-3 md:p-4 lg:col-span-2">
          ...
          <div className="flex flex-wrap gap-6 md:gap-8">
            {[
              { label: "TOTAL QUESTS", value: (generalMissions.length).toString() },
              { label: "COMPLETED", value: (apiMissions.length > 0
                ? (generalMissions as Mission[]).filter((m) => m.status === 'completed').length
                : generalMissions.filter((m: any) => m.status === 'COMPLETE').length
              ).toString() },
              ...
              { label: "EVENTS", value: generalEvents.length.toString() },
            ].map((s) => (
              <div key={s.label}>
