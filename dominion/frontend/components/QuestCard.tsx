    ...
    <div className="quest-scroll p-4 mb-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span
            className="text-[7px] px-2 py-0.5 font-bold"
            style={{ backgroundColor: pColor + "33", color: pColor }}
          >
            {mission.priority}
          </span>
          <span className="text-[8px]">{statusLabels[mission.status]}</span>
        </div>
        <span className="text-lg">{general?.emoji}</span>
      </div>

      {/* Title */}
      <h3 className="text-[10px] text-throne-gold mb-2">{mission.title}</h3>
      <p className="text-[7px] text-gray-400 mb-3">{mission.description}</p>
      ...
      {/* Footer */}
      <div className="flex justify-between text-[7px] text-gray-600 mt-3">
