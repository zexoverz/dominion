...
export default function GeneralCard({ general }: { general: General }) {
  ...
  return (
    <Link href={`/generals/${general.id}`}>
      <div
        className={`pixel-border-thin p-4 hover:translate-y-[-2px] transition-transform cursor-pointer ${
      ...
      >
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[8px] uppercase" style={{ color: general.color }}>
                {general.status}
              </span>
            </div>
            <h3 className="text-[10px] truncate" style={{ color: general.color }}>
              {general.name}
            </h3>
            <p className="text-[7px] text-gray-500 truncate">{general.title}</p>
          </div>
        </div>

        {/* Model badge */}
        <div className="flex items-center gap-2 mb-2">
          <span className="text-[7px] px-2 py-0.5 bg-throne-purple text-throne-goldLight">
            {general.model}
          </span>
        ...
        {general.currentMission ? (
          ...
          <p className="text-[7px] text-gray-600">💤 Awaiting orders...</p>
        )}
      </div>
    </Link>
  );
