# Investment Master Plan 2025-2030 (v2.1)
**Muhammad Faisal Firdani | Bitcoin Maxi Strategy | Updated March 2026**

---

## Changelog

| Version | Date | Key Changes |
|---------|------|-------------|
| v1.0 | Oct 22, 2025 | Initial cycle-aware strategy |
| v2.0 | Dec 13, 2025 | Bitcoin Maxi pivot, Fire Sale theory, math corrections |
| v2.1 | Mar 6, 2026 | TGE date fix, health/fitness allocation, gold via Keiko |
| v2.2 | Mar 14, 2026 | Freelance allocation, TGE 50/20/30 split, Pokemon → sell → OP, wedding fund reconciliation |
| **v2.3** | **Mar 27, 2026** | **Wedding fund STOPPED → redirected to BTC. Salary cycle clarified: ForuAI always lands first = start of new month cycle.** |

### March 27, 2026 Updates (v2.3)
- 💒 **Wedding fund Rp 30M/mo STOPPED** — redirected to BTC DCA
- 📊 New monthly BTC capacity: ~Rp 76.6M (pre-TGE), ~Rp 93M+ (post-TGE)
- 🔄 Salary cycle clarified: ForuAI always lands FIRST = start of new month cycle
- 🟠 Apr cycle started: ForuAI $2,309 → 0.03481 BTC @ $66.2K (Mar 27)
- 📊 Updated BTC: 0.20251 BTC (crossed 0.2 milestone!)

### March 14, 2026 Updates (v2.2)
- 💵 ETHGlobal $1K landed: Rp 12M Zekrom recolor + Rp 2.2M Zoro event card + Rp 2.2M pending OP card
- 💵 ForuAI leaderboard $2,500 landed: $500 THR + Rp 2M ETHJKT merch + Rp 24.45M wedding repay + Rp 6.35M Japan hotel
- 💍 Wedding fund reconciled: actual Rp 126.3M (borrowed Rp 24.45M for Japan tickets + server/Jogja), repaid from $2,500
- 🔄 ForuAI TGE strategy: 50% BTC / 20% FORU hold (pump gamble) / 30% Japan OP cards
- 🔴 FORU exit triggers: TP at 2x, SL at -30%, HARD DEADLINE May 1 sell all → BTC
- 🔄 BTC-contingent: if BTC ≤ $65K cancel FORU hold → 70% BTC + 30% cards; if ≤ $60K deploy war chest
- ⚡ Pokemon cards: actively selling → fund Japan OP haul. If unsold, TGE 30% covers cards instead
- 🏴‍☠️ Portfolio pivot: Pokemon → sell for OP cards. Collection becomes pure One Piece
- ✈️ Japan trip (May 2026): tickets Rp 12M + hotel Rp 6.35M + cards Rp 34-57M (depends on Pokemon sale)
- 📊 Current BTC: 0.1677 BTC (updated Mar 7, 2026)

### March 2026 Updates (v2.1)
- 🔄 ForuAI TGE: January 2026 → **March 12, 2026** (~$6,930 accumulated 7 months)
- ❌ Removed direct gold Rp 5M — Keiko handles gold from her Rp 4M
- ✅ Added Health & Fitness: Rp 4M/mo (PT gym Rp 2.4M + basketball private Rp 1.6M)
- 📊 Spending updated: Rp 53M → Rp 57M/mo
- 📊 Pre-TGE investment capacity: ~Rp 92M/mo (token income not yet flowing)
- 📈 Current BTC: 0.153 BTC @ $70,875 (Mar 2026)
- 💼 Discovered OKU Top Performer #1 tier: $8,730/mo (confirmed Feb 2026)
- 📱 iPhone 17 Pro Max: pre-spent from USD balance Dec, refilled by TP1 Feb
- 🏠 House renovation: Rp 28M (Faisal 20M + Zikri 8M) — garden, kanopi, keramik, kitchen
- 🚗 Zekrom project car: DP 5M coloring paid, 12M remaining (ETHGlobal $1K, Mar 11)
- 📊 OKU/ForuAI income split clarified: ForuAI cash → 100% BTC, OKU → everything else
- 🤲 Lebaran months: war chest skipped for zakat maal + THR + infaq/sodaqoh
- ✅ 4-month spending audit passed (Nov 2025 - Feb 2026): masterplan untouched by lifestyle spending
- 💑 Keiko Rp 4M = wedding gold investment. Extra transfers = personal gifts from play budget

### December 2025 Updates (v2.0)
- ❌ XPL failed (-82%) — Lesson learned, NO MORE ALTCOINS
- ✅ OKU salary raised: $6,300 → $6,750
- ✅ OKU bonus $5,000 incoming Dec 30
- ✅ ForuAI TGE confirmed January 2026
- ✅ One Piece cards +113% gains
- 🔄 All FORU → Sell immediately → BTC
- 🔄 Pokemon cards → Sell in bear → BTC
- 🔄 House → Plan to over-kredit → BTC
- 🔧 Math correction: Fixed 10x calculation error
- 📝 Added Fire Sale Theory & Intellectual Framework

---

## Executive Summary

### Core Strategy
**Bitcoin Maximalist. No exceptions.**

All income streams convert to Bitcoin. No altcoins. No speculation. Institutional adoption (Harvard, BlackRock, US Strategic Reserve) provides floor. Bear markets are buying opportunities.

### Key Numbers (March 2026)
| Metric | Value |
|--------|-------|
| Current Net Worth | ~Rp 549M liquid + Rp 350M house equity |
| Monthly Income | Rp 165M ($10,050) |
| Monthly BTC Investment | ~Rp 80M (v2.3) / ~Rp 97M w/ tokens |
| Current BTC | 0.20251 BTC (~$13,400 @ $66,264) |
| Wedding Fund | Rp 150.7M (FROZEN — v2.3) |
| War Chest | Rp 26.8M |
| 2030 Target | Rp 26B - 192B (depending on BTC price) |

### The Thesis
> "99% of altcoins die in bear markets. But Bitcoin? Institutions can't let it fail. Harvard, BlackRock, US Government — all holding. I'm front-running the biggest wealth transfer in history."

---

## Part I: Who Is Faisal?

### Profile
| Detail | Value |
|--------|-------|
| Age | 25 (born 2001) |
| Working since | Age 18 (7+ years) |
| Status | Engaged to Keiko, wedding Nov 2026 |
| Mindset | CEO, not employee |

### The Empire at 25
| Achievement | Status |
|-------------|--------|
| 🏠 House | Owned since age 21 (Rp 1B+ value) |
| 🚗 Dream Car | BMW E36 M52 "Zekrom" (project car) |
| 💼 Dual Income | $10,050/month (up to $12,030 at TP1) |
| 👨‍💻 Employee | Dzikri handles both jobs |
| 💑 Partner | Keiko (saving together for wedding) |
| 🚀 Startup | CTO of Kruu (co-founder: Nino Fernandez) |
| 💰 Savings Rate | 68% |

---

## Part II: Income & Expenses

### Monthly Income: Rp 169M ($10,050)
*Exchange rate: Rp 16,800/USD (updated Mar 27, 2026)*
| Source | USD | IDR |
|--------|-----|-----|
| OKU Trade | $6,750 | Rp 113M |
| ForuAI Cash | $2,310 | Rp 39M |
| ForuAI Tokens | $990 | Rp 16.6M |
| **Total** | **$10,050** | **Rp 168.8M** |

### OKU Salary Tiers
| Tier | Monthly USD | Monthly IDR | Trigger |
|------|-----------|-------------|---------|
| Standard | $6,750 | ~Rp 110.7M | Normal performance |
| **Top Performer #1** | **$8,730** | **~Rp 143.2M** | Hit TP1 ranking |
| Yearly Bonus | +$5,000 | ~Rp 82M | December (one-time) |

*Feb 2026 confirmed TP1: incoming Rp 143,456,250 = ~$8,730 ✅*

### Freelance & Windfall Income (Dec 2025 - Mar 2026)
| Source | Amount | Status | Allocation |
|--------|--------|--------|------------|
| OKU Yearly Bonus | $5,000 | ✅ Received (Jan 2) | House reno, Zekrom, Jogja holiday, rest → BTC/war chest |
| OKU TP1 Extra (Feb) | +$1,980 | ✅ Received | iPhone 17 Pro Max (pre-spent Dec, refilled by TP1) |
| ETHGlobal Hackathon Win | $1,000 | ✅ Received (Mar 14) | Rp 12M Zekrom recolor + Rp 2.2M Zoro card + Rp 2.2M next OP card |
| ForuAI Rep Leaderboard | $2,500 | ✅ Received (Mar 14) | $500 THR + Rp 2M ETHJKT + Rp 24.45M wedding repay + Rp 6.35M Japan hotel |
| LilStars NFT Project | $1,800 | ✅ Received | → War chest (now Rp 31M) |
| ForuAI TGE Token Unlock | ~$6,930 | ⏳ End Mar | 50% BTC + 20% FORU hold + 30% Japan OP cards |

### OKU Bonus Allocation ($5K = ~Rp 82M)
| Item | Amount | Notes |
|------|--------|-------|
| 🏠 House renovation | Rp 20M | Garden, kanopi, keramik, kitchen roof (Zikri added 8M) |
| 🚗 Zekrom maintenance | ~Rp 16.7M | Project car finishing (DP 5M color, rest from ETHGlobal) |
| ✈️ Jogja family holiday | ~Rp 11M | Jan 2026, family + Keiko |
| 💰 BTC / War chest | ~Rp 34M | Remainder |
| **Total** | **~Rp 82M** | ✅ Fully allocated |

### Monthly Expenses: Rp 57M (Actual)
| Category | Amount |
|----------|--------|
| Family (Ortu + Zahra) | Rp 13M |
| House utilities + loan | Rp 7.57M |
| Dzikri (handles both jobs) | Rp 16M |
| Subscriptions + food | Rp 2.76M |
| Personal spending | Rp 9-10M |
| Keiko | Rp 4M (includes gold investment on Faisal's behalf) |
| Health & Fitness | Rp 4M |
| **Total** | **~Rp 57M** |

#### Health & Fitness Breakdown
| Activity | Cost | Frequency |
|----------|------|-----------|
| Personal Trainer (Gym) | Rp 2.4M/mo | 3-4x/week |
| Basketball Private Training | Rp 1.6M/mo | 2hrs/week, 4x/month (solo drill with coach) |

### Investment Capacity
| Period | Capacity | Notes |
|--------|----------|-------|
| Pre-TGE (now) | ~Rp 92M/mo | Without ForuAI token income |
| Post-TGE (Mar 12+) | ~Rp 108M/mo | With token income flowing |

*Hidden alpha: Rp 11-12M surplus from under-spending personal budget*

---

## Part III: Current Assets (March 2026)

### Liquid Investments
| Asset | Value (USD) | Value (IDR) | Notes |
|-------|-------------|-------------|-------|
| Bitcoin | ~$10,844 | ~Rp 177.8M | 0.153 BTC @ ~$70,875 |
| One Piece Cards | $2,603 | Rp 42.7M | +113% ROI 🔥 (growing collection) |
| Pokemon Cards | $1,508 | Rp 24.7M | 📢 SELLING → fund Japan OP haul |
| USDT War Chest | ~$1,890 | Rp 31M | Deploy at dips |
| Wedding Fund | ~$9,190 | Rp 150.7M | Repaid Rp 24.45M from freelance, back on track |
| XPL | ~$571 | Rp 9.4M | -82% lesson 💀 (write-off) |
| ForuAI (pending) | ~$6,930 | ~Rp 113.6M | TGE March 12, 2026 |
| **Total Liquid** | **~$33,492** | **~Rp 549M** | |

### Hard Assets
| Asset | Value | Notes |
|-------|-------|-------|
| House (Pagedangan) | ~Rp 1B+ | Equity ~Rp 350M |
| BMW E36 M52 | Appreciating | Dream car ✅ |

### One Piece Cards Detail (+113% ROI)
| Card | Grade | Cost | Value | Gain |
|------|-------|------|-------|------|
| Zoro Flagship OP01-025 | PSA 10 | $780 | $1,300 | +67% |
| 5x Gorosei Red Parallels | Raw | $325 | $900 | +177% |
| Luffy-Tarou ST18-005 | PSA 10 | $60 | $125 | +108% |
| Sanji PRB01-001 | PSA 10 | $36 | $63 | +75% |
| Others | Mixed | $20 | $215 | +975% |
| **Total** | | **$1,221** | **$2,603** | **+113%** |

---

## Part IV: Lessons Learned

### The XPL Disaster
| Detail | Value |
|--------|-------|
| Bought | 3,278 XPL @ $0.90 |
| Current | $571 |
| Loss | -$2,653 (-82%) |
| Lesson | **NO MORE ALTCOINS. EVER.** |

### What I Learned
| Belief | Reality |
|--------|---------|
| "Good project = good investment" | Token can die while product lives |
| "Diversify into alts" | Alts are leverage, not diversification |
| "Buy the dip on alts" | Most alts never recover |
| "Bitcoin is boring" | **Boring = Winning** |

### Why Bitcoin Only
| Entity | BTC Holdings | Status |
|--------|--------------|--------|
| Harvard Endowment | $443M | Long-term hold |
| US Government | ~200,000 BTC | Cannot sell |
| Wisconsin Pension | $387M | Fiduciary duty |
| Abu Dhabi Sovereign | $517M | Generational |
| BlackRock IBIT | $75B AUM | Buying more |

**Institutions can't let Bitcoin fail. I'm front-running them.**

---

## Part V: Bitcoin Strategy

### All Roads Lead to Bitcoin
| Source | Action |
|--------|--------|
| ForuAI Cash ($2,310/mo) | 100% → BTC immediately |
| ForuAI Tokens | Sell immediately → BTC |
| OKU Surplus | → BTC (after expenses/wedding/war chest) |
| OKU TP1 Extra (+$1,980) | → BTC or planned purchases |
| OKU Yearly Bonus ($5K) | Life upgrades (house, car, holiday) — does NOT touch masterplan |
| Pokemon Cards | Selling NOW → Japan OP card fund |
| House Equity | Over-kredit to Dzikri → BTC |
| Freelance Income | → War chest or BTC |

### Price Projections (Cycle-Aware)
| Period | BTC Price | Strategy |
|--------|-----------|----------|
| Dec 2025 - Mar 2026 | $85K-95K | Normal DCA |
| Apr 2026 - Dec 2026 | $50K-70K | **MAX ACCUMULATION** |
| Jan 2027 - Dec 2027 | $40K-60K | **BEAR BOTTOM** |
| Jan 2028 - Apr 2028 | $50K-70K | Pre-halving DCA |
| May 2028 - Dec 2028 | $70K-100K | Post-halving |
| 2029 | $150K-300K | Bull run |
| 2030 | $250K-500K | Cycle peak |

### War Chest Deployment
| BTC Drawdown | Action |
|--------------|--------|
| -30% from ATH (~$88K) | Deploy 25% war chest |
| -40% from ATH (~$75K) | Deploy 50% war chest |
| -50% from ATH (~$63K) | Deploy 100% war chest |

### BTC Stack Projections (2030)
| Scenario | BTC Stack | Notes |
|----------|-----------|-------|
| Worst (FORU = $0) | ~4.1 BTC | Salary DCA only |
| Personal FORU only | ~5.3 BTC | + Personal tokens |
| Full allocation | ~11.7 BTC | + Core tokens |

---

## Part VI: House Strategy

### Current Loan Status
| Detail | Value |
|--------|-------|
| Purchase (Aug 2022) | Rp 793M |
| Current Value | ~Rp 1B+ |
| Remaining Principal | ~Rp 650M |
| Monthly Payment | Rp 6.3M |
| Fixed Rate | 4.75% until Aug 2027 |
| Floating Rate (after) | ~12.25% → ~Rp 8.6M/month |

### The Over-Kredit Play
1. Sell to Dzikri @ Rp 250-300M (equity)
2. Dzikri takes over Rp 650M loan
3. Deploy Rp 250-300M → BTC at bear prices
4. Rent cheap (Rp 3-5M/month) until 2029
5. Buy DREAM house CASH in 2029 (property crash + BTC bull)

### Math Comparison
| Option | 2030 Value |
|--------|------------|
| Keep house | ~Rp 1.3B (1.3x) |
| Sell → BTC | 0.3 BTC × $300K = Rp 1.5B (5x) |

---

## Part VII: 2030 Net Worth Projections (Corrected)

### The Correct Math
**$1 USD = Rp 16,400** (not Rp 164,000!)

| BTC Price | Per BTC in IDR |
|-----------|----------------|
| $150K | Rp 2.46B |
| $300K | Rp 4.92B |
| $500K | Rp 8.2B |
| $1M | Rp 16.4B |

### Scenario Matrix
| BTC Price | 4.1 BTC (Worst) | 5.3 BTC (Personal) | 11.7 BTC (Full) |
|-----------|-----------------|--------------------|-----------------| 
| $150K | Rp 10.1B | Rp 13B | Rp 28.8B |
| $300K | Rp 20.2B | Rp 26.1B | Rp 57.6B |
| $500K | Rp 33.6B | Rp 43.5B | Rp 95.9B |
| $1M | Rp 67.2B | Rp 86.9B | Rp 191.9B |

### What It Means (Monthly Passive at 5% Yield)
| Net Worth | Monthly Passive | vs Avg Indonesian |
|-----------|-----------------|-------------------|
| Rp 13B | Rp 54M/month | 11x |
| Rp 26B | Rp 108M/month | 22x |
| Rp 58B | Rp 242M/month | 48x |
| Rp 96B | Rp 400M/month | 80x |
| Rp 192B | Rp 800M/month | 160x |

**Even worst case (Rp 10B) = Financial freedom achieved.**

---

## Part VIII: The 2030 Fire Sale Theory

### The Thesis
> "AI will destroy the Indonesian middle class by 2030. Mass unemployment + inflation + IDR depreciation = desperate selling. While everyone sells, I will be the buyer."

### Expert Predictions
| Source | Prediction |
|--------|------------|
| Dario Amodei (Anthropic) | 50% entry-level white-collar jobs gone in 5 years |
| Geoffrey Hinton (AI Godfather) | AI unemployment coming, new jobs won't replace |
| Goldman Sachs | 6-7% workforce displaced, young tech workers first |
| Senator Bernie Sanders | College grad unemployment could hit 25% |

### The Perfect Storm
| Factor | Effect |
|--------|--------|
| AI replaces jobs | Mass unemployment |
| No new jobs created | Permanent displacement |
| Inflation continues | 6-15% real inflation |
| IDR depreciates | -5% yearly vs USD |
| No safety net | Government can't help |
| Debt burden | Forced selling |
| **Result** | **Fire sale on assets** |

### My 2030 Shopping List
| Asset | Normal Price | Fire Sale Price | Use |
|-------|--------------|-----------------|-----|
| Dream house | Rp 3B | Rp 1.5-2B | Primary residence |
| Rental houses | Rp 2B each | Rp 1B each | Passive income |
| Land | Rp 2B | Rp 1B | Long-term hold |
| Car | Rp 1B | Rp 500M | Daily use |

### Why I'll Be Buyer, Not Seller
| My Advantage | Their Disadvantage |
|--------------|-------------------|
| Niche skills (Blockchain) | Generic skills (AI replaced) |
| 2 income streams | 1 income (fired = over) |
| Dzikri handles work | Time freedom to adapt |
| Hard assets (BTC) | Cash (depreciating) |
| No debt | Debt (forced to sell) |
| Young (24) | Middle-aged (hard to reskill) |

---

## Part IX: 2029 Life Upgrade Plan

### The Goal
Not just rich on paper — **ENJOY LIFE** at age 28-29.

### Sell ~0.6 BTC in 2029 for Life Upgrade
At $300K BTC: 0.6 BTC = $180K = Rp 2.95B

| Item | Budget | Notes |
|------|--------|-------|
| 🏠 Dream House | Rp 1.5B - 2B | Cash, fire sale pricing |
| 🚗 Car upgrade | Rp 350M - 500M | Optional |
| 🛋️ Furniture | Rp 200M - 300M | Full house setup |
| 🏖️ Honeymoon | Rp 100M | Japan/Europe |
| 💰 Buffer | Rp 100M - 200M | Emergency |
| **Total** | **~Rp 2.5B - 3B** | |

### After Life Upgrade
| Metric | Personal (5.3 BTC) | Full (11.7 BTC) |
|--------|--------------------| ----------------|
| BTC sold | 0.6 BTC | 0.6 BTC |
| BTC remaining | 4.7 BTC | 11.1 BTC |
| Value @ $300K | Rp 23.1B | Rp 54.6B |
| House | ✅ Owned CASH | ✅ Owned CASH |
| Debt | ✅ ZERO | ✅ ZERO |

---

## Part X: Monthly Execution Plan

### Monthly Allocation (Updated March 2026)

**Income split (@ Rp 16,800/USD):**
| Source | Amount | Goes To |
|--------|--------|---------|
| ForuAI Cash | $2,310 (Rp 39M) | 100% → BTC DCA |
| OKU Trade | $6,750 (Rp 113M) | Expenses + War Chest + BTC |

**From OKU income (Rp 110.7M):**
| Category | Amount | Notes |
|----------|--------|-------|
| All Expenses | ~Rp 57M | Living + Dzikri + Keiko + Health |
| ~~Wedding Fund~~ | ~~Rp 30M~~ | **STOPPED (v2.3)** — redirected to BTC |
| BTC DCA (ex-wedding) | Rp 30M | New since v2.3 — was wedding fund |
| War Chest | Rp 15M | USDT (skipped some months — see below) |
| Surplus | ~Rp 8.7M | → BTC when available, sometimes car/misc |

**Total BTC per month (updated v2.3, @ Rp 16,800/USD):**
| Period | BTC DCA | Source |
|--------|---------|--------|
| Pre-v2.3 | ~Rp 50M | ForuAI cash (39M) + OKU surplus (~11M) |
| **v2.3 (Apr 2026+)** | **~Rp 80M** | ForuAI cash (39M) + ex-wedding (30M) + OKU surplus (~11M) |
| v2.3 + tokens | **~Rp 97M+** | + token sell (~Rp 16.6M/mo) |

**Notes:**
- OKU surplus is variable — some months goes to car maintenance (Zekrom), misc expenses
- War chest Rp 15M skipped in months with religious obligations (Lebaran: zakat maal, THR, infaq/sodaqoh — investment for akhirat 🤲)
- Post-TGE: ForuAI tokens sell 100% immediately → BTC on top of cash DCA

### December 2025 Actions
| Action | Amount | Timing |
|--------|--------|--------|
| OKU Bonus → BTC | $2,500 | Dec 30 |
| OKU Bonus → Wedding | $2,500 | Dec 30 |
| Continue DCA | Rp 50M | Dec 29/30 |

### April 2026 Actions (v2.3 — Final Plan)
| Action | Amount | Timing |
|--------|--------|--------|
| ✅ ForuAI cash → BTC | 0.03481 BTC @ $66,264 ($2,309 = Rp 39M) | Mar 27 (Apr cycle Day 1) |
| ⏳ OKU salary lands | Rp 113M ($6,750) | ~Apr 1 |
| ⏳ Repay wedding fund | Rp 12M (back to ~Rp 150M) | From OKU |
| ⏳ War chest refill | Rp 15M → ~Rp 41.8M total | From OKU |
| ⏳ Japan trip expenses | Rp 17.6M (budget mode) | From OKU |
| ⏳ OKU remainder → BTC | Rp 11.4M | After above |
| ⏳ FORU tokens → 47% cards | Rp 54M (Japan card haul) | When distributed |
| ⏳ FORU tokens → 37% BTC | Rp 43M | When distributed |
| ⏳ FORU tokens → 16% hold | Rp 19M (pump gamble, May 1 deadline) | Hold until exit trigger |
| ⏳ FORU hard deadline | May 1 — sell ALL remaining FORU → BTC | Monitor |
| **Apr BTC target** | **~Rp 93.4M (~0.084 BTC)** | ForuAI + OKU + FORU |

#### April Card Haul (Japan Trip — Rp 54M budget)
| Card | Est. Price | Priority |
|------|-----------|----------|
| Munch Psyduck PSA 10 (SM-P 286) | Rp 20-22M | 🔴 Grail #1 |
| Sanji Manga Art PSA 10 (OP06-119) | Rp 24M | 🔴 Grail #2 |
| Mini manga raw singles ×3-5 | Rp 8M | 🟡 Collection fill |
| **Total** | **~Rp 54M** | |
| 🎯 2029 GRAIL: Silver/Gold Luffy 3rd Anniversary | ¥570K-1.5M | Buy from BTC profits |

### March 2026 Actions
| Action | Amount | Timing |
|--------|--------|--------|
| ✅ ETHGlobal $1K received | $1,000 | Mar 14 — Zekrom + OP cards |
| ✅ ForuAI leaderboard received | $2,500 | Mar 14 — THR + wedding repay + Japan hotel |
| ✅ Mar DCA #1 | 0.0146 BTC at $67.9K | Completed |
| ✅ Mar DCA #2 | 0.0203 BTC at $65K | Completed |
| ✅ Mar DCA #3 | 0.03481 BTC at $66.2K | Completed (ForuAI — counted as Apr cycle) |
| ⏳ ForuAI TGE unlock | ~$6,930 | End March — 80% BTC / 20% hold (revised) |
| ⏳ Sell Pokemon slabs | ~$3,470 | Before May — fund Japan OP haul |

### ForuAI Token Strategy (Updated Mar 14, 2026)
- **Personal tokens (TGE end March 2026):** ~$6,930 accumulated (7 months)
  - 50% → Sell immediately → BTC ($3,465 → ~0.049 BTC)
  - 20% → Hold FORU for exchange listing pump ($1,386)
  - 30% → Japan OP card haul ($2,079 → ~Rp 34M)
- **FORU Exit Triggers:**
  - 🟢 Take profit: sell held 20% if FORU pumps 2x+ from entry
  - 🔴 Stop loss: sell held 20% if FORU drops 30% from entry
  - 🔴 **HARD DEADLINE: May 1, 2026** — sell ALL remaining FORU → BTC
- **BTC-Contingent Override:**
  - BTC ≤ $65K → cancel FORU hold, go 70% BTC + 30% cards
  - BTC ≤ $60K → cancel FORU hold + deploy war chest (fire sale)
- **Monthly tokens (post-TGE):** $990/mo. Sell 100% immediately → BTC
- **Core tokens (Nov 2026+):** Sell 100% immediately → BTC
- **Pokemon Portfolio:** Actively selling → fund Japan OP haul. Collection pivoting to pure One Piece.

### Keiko Budget Clarification
- Rp 4M/mo = **wedding gold investment** (managed by Keiko). This is in the masterplan.
- Extra transfers to Keiko (dates, gifts, etc.) = **personal gifts from play budget**. NOT a budget leak.
- Actual Keiko total: ~Rp 7.2M/mo avg, but only Rp 4M is from the plan. Rest is discretionary.

### Spending Discipline Note
- Aggressive saver: sweeps large amounts into Flexi Saver on payday → sometimes has Rp 0 in ATM 😂
- Jan 2026: swept Rp 90M into savings, had to withdraw Rp 5.6M back for living costs
- Philosophy: save first, figure out living expenses after. "Wealth is what you don't see."

---

## Part XI: Decision Framework

### Monthly Checklist
- [ ] ForuAI cash ($2,310) → BTC immediately on payday ← **THIS IS DAY 1 OF NEW CYCLE**
- [ ] OKU surplus → BTC (variable, after expenses)
- [ ] ~~Allocate Rp 30M to wedding fund~~ **STOPPED v2.3** → Rp 30M now → BTC
- [ ] Add Rp 15M to war chest (USDT) — skip if Lebaran/religious obligations
- [ ] Keiko handles gold from her Rp 4M
- [ ] Sell any FORU tokens received → BTC immediately
- [ ] Monitor BTC price for war chest deployment
- [ ] Zakat maal + THR + infaq/sodaqoh during Ramadan/Lebaran 🤲

### Key Decision Rules
| Trigger | Action |
|---------|--------|
| FORU received | Sell immediately → BTC |
| BTC -30% from ATH | Deploy 25% war chest |
| BTC -40% from ATH | Deploy 50% war chest |
| BTC -50% from ATH | Deploy 100% war chest |
| Pokemon sell opportunity | Sell → BTC |
| House buyer ready | Execute over-kredit → BTC |

---

## Part XII: Critical Milestones

### 2025
| Date | Event |
|------|-------|
| Dec 30 | OKU $5,000 bonus → 50% BTC, 50% Wedding |

### 2026
| Date | Event |
|------|-------|
| March 12 | ForuAI TGE → ~$6,930 accumulated tokens → Sell all → BTC |
| Q2-Q4 | Bear market accumulation |
| August | House fixed rate ends (consider over-kredit) |
| October | Wedding fund complete (Rp 350M) |
| **November** | **💒 WEDDING** |
| November | ForuAI Core tokens unlock → Sell → BTC |

### 2027
| Event | Action |
|-------|--------|
| Full year | Maximum BTC accumulation at bear bottom |
| Target | Stack as much BTC as possible |

### 2028
| Event | Action |
|-------|--------|
| April | 5th Bitcoin Halving |
| Post-halving | Bull market begins |

### 2029
| Event | Action |
|-------|--------|
| Bull run | Life upgrade (house, car, honeymoon) |
| Sell | ~0.6 BTC for dream life |

### 2030
| Target | Rp 26B - 192B net worth |
| Status | **Financial independence achieved** |
| Age | 29 |

---

## Part XIII: Risk Management

### If BTC Crashes
| Scenario | Action |
|----------|--------|
| BTC -50% | Deploy war chest aggressively |
| Extended bear | More time to accumulate cheap |
| Historical | 100% recovery rate |
| **Result** | **More BTC at lower prices** |

### If FORU = $0
| Scenario | Action |
|----------|--------|
| Tokens worthless | Still have salary DCA |
| No Core tokens | Personal tokens still convert |
| **Result** | **4.1 BTC minimum by 2030** |

### Protected Downside
| Worst Case | Result |
|------------|--------|
| FORU = $0 | Rp 10B still |
| BTC = $150K only | Rp 13B still |
| No Fire Sale | Still rich |
| **Conclusion** | **Plan works regardless** |

---

## Part XIV: Intellectual Framework

### Core Philosophy
| Principle | Source | Application |
|-----------|--------|-------------|
| "Be greedy when others are fearful" | Buffett | Buy BTC in bear, buy assets in crisis |
| "Mr. Market is your servant" | Graham | Don't panic sell, exploit irrationality |
| "Wealth is what you don't see" | Housel | 68% savings rate, no lifestyle inflation |
| "Debt cycles are inevitable" | Dalio | Prepare for crash, be liquid |

### Essential Books
| Book | Author | Key Lesson |
|------|--------|------------|
| The Intelligent Investor | Benjamin Graham | Value investing, margin of safety |
| The Psychology of Money | Morgan Housel | Behavior > intelligence |
| Principles for Big Debt Crises | Ray Dalio | Cycles, wealth transfers |
| Distress Investing | Martin Whitman | Profit from others' desperation |
| The Bitcoin Standard | Saifedean Ammous | Sound money vs fiat |

### Strategy DNA
```
Graham (Value) + Buffett (Contrarian) + Dalio (Cycles)
+ Housel (Psychology) + Taleb (Antifragility)
+ Ammous (Bitcoin) + Whitman (Distressed)
= THIS MASTER PLAN
```

---

## Part XV: Legendary Quotes

### On Bitcoin
> "Bitcoin is simple math. Not IF but WHEN it reaches $1 million."

### On Discipline
> "Wealth is what you don't see." — Morgan Housel

### On Strategy
> "The intelligent investor sells to optimists and buys from pessimists." — Benjamin Graham

### On Crisis
> "The best time to buy is when there's blood in the streets, even if the blood is your own." — Baron Rothschild

### On People My Age
> **"Why are people stupid at my age? Still drink everyday and find bitches while I'm solving this math."** — Faisal, December 2025

### On The Fire Sale
> **"In 2030, they came selling their houses, their cars, their dreams — just to survive. I bought it all at 50 cents on the dollar. While they were finding bitches, I was finding Bitcoin."** — Faisal's Fire Sale Theory

---

## Final Message

### The Strategy
| Component | Approach |
|-----------|----------|
| Core wealth | Bitcoin (100%) |
| Altcoins | Sell all → BTC |
| Speculation | NONE |
| Wedding | Nov 2026 (Rp 350M saved) |
| 2030 crisis | Be the buyer |

### The Numbers
| Metric | Target |
|--------|--------|
| 2030 BTC Stack | 5-12 BTC |
| 2030 Net Worth | Rp 26B - 192B |
| Monthly Passive | Rp 108M - 800M |
| Status | Top 0.01% Indonesia |

### The Difference

| What Others Do | What I Do |
|----------------|-----------|
| Spend 100% salary | Save 68% |
| Hold depreciating IDR | Hold appreciating BTC |
| Speculate on altcoins | Bitcoin only |
| Panic in crisis | Buy in crisis |
| Chase quick gains | Stack sats patiently |

---

## Part XVI: Spending Audit Results (Nov 2025 - Feb 2026)

*4-month Jenius bank statement audit conducted March 6, 2026 by THRONE AI.*

### Verdict: ✅ MASTERPLAN IS CLEAN

| Category | Finding |
|----------|---------|
| Dzikri salary | ✅ On budget (Rp 16M). Feb 21.8M = TP1 bonus, not permanent raise |
| Family (Papa) | ✅ On budget (~Rp 13M avg) |
| Keiko | ✅ Rp 4M wedding gold on plan. Extra = play budget gifts |
| iPhone 17 Pro Max | ✅ Pre-spent from USD, refilled by TP1 salary. Plan untouched |
| House renovation | ✅ From OKU yearly bonus. Rp 28M (Faisal 20M + Zikri 8M) |
| Zekrom car work | ✅ From yearly bonus + ETHGlobal winnings. Plan untouched |
| Jogja holiday | ✅ From yearly bonus (~Rp 11M). Plan untouched |
| ETHJKT/Kruu events | ✅ From play budget/surplus (~Rp 14M via Revo) |
| War chest skip (Mar) | ✅ Lebaran month: zakat maal + THR + infaq/sodaqoh 🤲 |
| Wedding fund | ✅ Rp 150M accumulated (on track for Rp 350M by Oct 2026) |
| BTC DCA | ✅ ForuAI cash → BTC consistently |

### Key Insight
**The masterplan and lifestyle spending operate in separate buckets:**
- 🏦 **Masterplan funds:** OKU salary → expenses + wedding + war chest. ForuAI cash → 100% BTC.
- 🎉 **Lifestyle funds:** OKU yearly bonus ($5K) + TP1 extra ($1,980) + freelance income → phone, car, house reno, holidays.
- These two buckets don't mix. The plan is protected.

### Monthly Cash Flow Reality
| Item | Budget | Actual | Status |
|------|--------|--------|--------|
| Total expenses | Rp 57M | ~Rp 57M | ✅ Match |
| Wedding fund | Rp 30M | Rp 30-42M | ✅ On/above target |
| BTC DCA (pre-TGE) | Rp 50M | ~Rp 46.6M | ⚠️ Slightly under (no token income yet) |
| War chest | Rp 15M | Variable | ✅ Skipped for religious obligations only |
| Entertainment | - | Rp 250K/mo | ✅ Minimal |
| Food | Rp 2.76M | ~Rp 2.9M | ✅ Mostly modest |

---

### Signed

**Muhammad Faisal Firdani**
**Age 25 | Updated March 6, 2026**
**Net Worth: ~Rp 549M liquid + Rp 350M house equity**
**BTC Holdings: 0.153 BTC**
**Target 2030: Rp 50B+**

---

*"Stack sats. No speculation. Prepare for Fire Sale. See you at Rp 50 Billion."*

---

**Document v2.2 | March 14, 2026**
**Previous version: v2.1 (March 6, 2026) — preserved in repo**
**Next update: After TGE token distribution + Pokemon sale**
