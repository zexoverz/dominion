...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO ops_mission_proposals (agent_id, title, description, priority, estimated_cost_usd, proposed_steps, metadata)
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE ops_mission_proposals SET status = $1, rejection_reason = $2, reviewed_at = NOW() WHERE id = $3 RETURNING *`,
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM ops_mission_proposals WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Proposal not found' });
    }
