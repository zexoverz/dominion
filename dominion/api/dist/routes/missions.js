...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const mission = await db_1.default.query('SELECT * FROM ops_missions WHERE id = $1', [id]);
        if (mission.rows.length === 0) {
            return res.status(404).json({ error: 'Mission not found' });
        }
        const steps = await db_1.default.query('SELECT * FROM ops_mission_steps WHERE mission_id = $1 ORDER BY step_order', [id]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const proposal = await db_1.default.query('SELECT * FROM ops_mission_proposals WHERE id = $1', [proposal_id]);
        if (proposal.rows.length === 0) {
            return res.status(404).json({ error: 'Proposal not found' });
        }
        const p = proposal.rows[0];
        const result = await db_1.default.query(`INSERT INTO ops_missions (proposal_id, agent_id, title, description, priority, estimated_cost_usd)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [p.id, p.agent_id, p.title, p.description, p.priority, p.estimated_cost_usd]);
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        params.push(id);
        const result = await db_1.default.query(`UPDATE ops_missions SET ${sets.join(', ')} WHERE id = $${params.length} RETURNING *`, params);
...
router.post('/:id/steps', async (req, res) => {
    try {
        ...
        const maxOrder = await db_1.default.query('SELECT COALESCE(MAX(step_order), 0) as max_order FROM ops_mission_steps WHERE mission_id = $1', [id]);
        let order = maxOrder.rows[0].max_order;
        const created = [];
        for (const s of steps) {
            order++;
            const result = await db_1.default.query(`INSERT INTO ops_mission_steps (mission_id, agent_id, step_order, kind, title, description, input_data)
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [id, mission.rows[0].agent_id, order, s.kind || 'action', s.title, s.description || '', JSON.stringify(s.input_data || {})]);
...
router.patch('/:mid/steps/:sid', async (req, res) => {
    try {
        ...
        params.push(sid);
        params.push(mid);
        const result = await db_1.default.query(`UPDATE ops_mission_steps SET ${sets.join(', ')} WHERE id = $${params.length - 1} AND mission_id = $${params.length} RETURNING *`, params);
        if (result.rows.length === 0) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query('DELETE FROM ops_missions WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Mission not found' });
        }
